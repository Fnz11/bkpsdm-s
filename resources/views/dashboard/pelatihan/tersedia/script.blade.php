<script>
    ...
    $('#filter-jenis, #filter-metode, #filter-pelaksanaan, #filter-start-date, #filter-end-date').on('change',
        function() {
            fetchData();
        });
    ...
    function getFilters() {
        return {
            ...
            metode: $('#filter-metode').val(),
            pelaksanaan: $('#filter-pelaksanaan').val(),
    ...
    function fetchData(page = 1) {
        ...
        $('#tersedia-table').addClass('d-none');
        $('#loading-spinner').removeClass('d-none');
        $('.pagination a').addClass('disabled');

        $.ajax({
            url: "{{ route('dashboard.pelatihan.tersedia') }}",
            type: "GET",
            data: {
                page: page,
                ...
                pelaksanaan: filters.pelaksanaan,
                start_date: filters.start_date,
                end_date: filters.end_date
            },
            success: function(response) {
                $('#tersedia-table').html($(response).find('#tersedia-table').html());
                $('#pagination-wrapper').html($(response).find('#pagination-wrapper').html());
                applyColumnToggles();
            },
            complete: function() {
                $('#tersedia-table').removeClass('d-none');
                $('#loading-spinner').addClass('d-none');
                $('.pagination a').removeClass('disabled');
            },
            error: function() {
                showAlertModal("Gagal memuat data, silakan coba lagi.", "Error", 4000);
            }
        });
    }
    ...
    document.getElementById('btn-reset-filter').addEventListener('click', function() {
        if ($('#filter-search').val().length > 0 ||
            ...
            document.getElementById('filter-pelaksanaan').value = '';
            // Reset tanggal
    ...
    // Ganti bagian delete event listener dengan ini:
    $(document).on('click', '.btn-delete', function() {
        const form = $(this).closest('form');

        if (form.length) {
            showAlertModal('Apakah Anda yakin ingin menghapus pelatihan ini?', 'Warning', 0, {
    ...
    $(document).on('click', '.btn-update', function() {
        ...
        if (form.length) {
            showAlertModal('Apakah Anda yakin ingin memperbarui status pelatihan?', 'Confirm', 0, {
                confirmText: '<i class="bi bi-check-circle me-1"></i>Perbarui',
                onConfirm: () => {
                    form.submit();
                }
            });
    ...
    $(document).ready(function() {
        ...
        function updateSummaryData() {
            ...
            $('#summary-metode').text($('#filter-modal-metode option:selected').text() || 'Semua');
            $('#summary-pelaksanaan').text($('#filter-modal-pelaksanaan option:selected').text() || 'Semua');
        ...
        function fetchDataModal() {
            ...
            let metode = $('#filter-modal-metode').val();
            let pelaksanaan = $('#filter-modal-pelaksanaan').val();
            let start = $('#filter-modal-start-date').val();
            let end = $('#filter-modal-end-date').val();

            $.ajax({
                url: "{{ route('dashboard.pelatihan.tersedia.preview') }}",
                type: "GET",
                data: {
                    search: search,
                    jenis: jenis,
                    ...
                    if (response.data.length > 0) {
                        response.data.forEach(item => {
                            ...
                            <tr>
                                ...
                                <td>${item.status_pelatihan ? item.status_pelatihan.charAt(0).toUpperCase() + item.status_pelatihan.slice(1) : '-'}</td>
                            </tr>
                        `);
                        });
                        ...
                        $('#input-pelaksanaan').val(pelaksanaan);
        ...
        $('#filter-modal-jenis, #filter-modal-metode, #filter-modal-pelaksanaan, #filter-modal-start-date, #filter-modal-end-date')
            .on('change', function() {
                fetchDataModal();
            });
        ...
        modal.addEventListener('hidden.bs.modal', function() {
            ...
            $('#filter-modal-metode').val('');
            $('#filter-modal-pelaksanaan').val('');
    ...
    $(document).ready(function() {
        ...
        if (modalExcel) {
            ...
            function updateSummaryDataExcel() {
                ...
                'Semua');
                $('#summary-metode-excel').text($('#filter-modal-metode-excel option:selected').text() ||
                    'Semua');
                $('#summary-pelaksanaan-excel').text($('#filter-modal-pelaksanaan-excel option:selected')
                .text() || 'Semua');
            ...
            function fetchDataModalExcel() {
                ...
                let metode = $('#filter-modal-metode-excel').val();
                let pelaksanaan = $('#filter-modal-pelaksanaan-excel').val();
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.data.length > 0) {
                            ...
                            $('#input-metode-excel').val(metode);
                            $('#input-pelaksanaan-excel').val(pelaksanaan);
            ...
            $('#filter-modal-jenis-excel, #filter-modal-metode-excel, #filter-modal-pelaksanaan-excel, #filter-modal-start-date-excel, #filter-modal-end-date-excel')
                .on('change', function() {
                    fetchDataModalExcel();
                });
            ...
            modalExcel.addEventListener('hidden.bs.modal', function() {
                ...
                $('#filter-modal-metode-excel').val('');
                $('#filter-modal-pelaksanaan-excel').val('');
