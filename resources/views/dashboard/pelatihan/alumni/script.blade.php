<script>
    ...
    $('#filter-unit, #filter-start-date, #filter-end-date').on('change',
        function() {
            fetchData();
        });
    ...
    function fetchData(page = 1) {
        $('#alumni-table').addClass('d-none');
        $('#loading-spinner').removeClass('d-none');
        $('.pagination a').addClass('disabled');

        $.ajax({
            url: "{{ route('dashboard.pelatihan.alumni') }}",
            type: "GET",
            data: {
            ...
            success: function(response) {
                $('#alumni-table').html($(response).find('#alumni-table').html());
                $('#pagination-wrapper').html($(response).find('#pagination-wrapper').html());
                applyColumnToggles();
            },
            complete: function() {
                $('#alumni-table').removeClass('d-none');
    ...
    $(document).ready(function() {
        ...
        function fetchDataModal() {
            ...
            $.ajax({
                url: "{{ route('dashboard.pelatihan.alumni.preview') }}",
                type: "GET",
                data: {
                    search: search,
                    unit_id: unit,
                ...
                success: function(response) {
                    ...
                    if (response.data.length > 0) {
                        response.data.forEach(item => {
                            tbody.append(`
                                <tr>
                                    ...
                                    <td>${item.laporan?.judul_laporan ?? '-'}</td>
                        ...
                        $('#preview-empty-text').text('Tidak ada data yang cocok.');
        ...
        $('#filter-modal-unit, #filter-modal-start-date, #filter-modal-end-date')
            .on('change',
                function() {
                    fetchDataModal();
                });
    ...
    $(document).ready(function() {
        ...
        if (modalExcel) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.data.length > 0) {
                            ...
                            $('#preview-empty-text-excel').text('Tidak ada data yang cocok.');
                            // showAlertModal('Tidak ada data yang cocok.', 'Warning');
                        }
                    },
                    error: function() {
            ...
            $('#filter-modal-unit-excel, #filter-modal-start-date-excel, #filter-modal-end-date-excel')
                .on(
                    'change',
                    function() {
                        fetchDataModalExcel();
