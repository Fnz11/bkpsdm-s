<script>
    ...
    $('#filter-jenis, #filter-unit, #filter-metode, #filter-pelaksanaan, #filter-start-date, #filter-end-date, #filter-min-cost, #filter-max-cost')
        .on('change',
            function() {
                fetchData();
            });
    ...
    function getFilters() {
        return {
            ...
            min_cost: $('#filter-min-cost').val(),
            max_cost: $('#filter-max-cost').val()
        };
    }
    ...
    function fetchData(page = 1) {
        ...
        $('#usulan-table').addClass('d-none');
        $('#loading-spinner').removeClass('d-none');
        $('.pagination a').addClass('disabled');

        $.ajax({
            url: "{{ route('dashboard.pelatihan.usulan') }}",
            type: "GET",
            data: {
                page: page,
            ...
            complete: function() {
                $('#usulan-table').removeClass('d-none');
    ...
    document.getElementById('btn-reset-filter').addEventListener('click', function() {
        if ($('#filter-search').val().length > 0 ||
            ...
            document.getElementById('filter-max-cost').value = '';
    ...
    $(document).ready(function() {
        ...
        function fetchDataModal() {
            ...
            $.ajax({
                url: "{{ route('dashboard.pelatihan.usulan.preview') }}",
                type: "GET",
                data: {
                    search: search,
                    jenis: jenis,
