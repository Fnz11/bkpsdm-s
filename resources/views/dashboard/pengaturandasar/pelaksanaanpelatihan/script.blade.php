...
<script>
    ...
    function fetchData(page = 1) {
        $('#pelaksanaanpelatihan-table').addClass('d-none');
        $('#loading-spinner').removeClass('d-none');
        $('.pagination a').addClass('disabled');

        $.ajax({
            url: "{{ route('dashboard.pelatihan.pelaksanaanpelatihan') }}",
            type: "GET",
            data: {
            ...
            success: function(response) {
                $('#pelaksanaanpelatihan-table').html($(response).find('#pelaksanaanpelatihan-table').html());
                $('#pagination-wrapper').html($(response).find('#pagination-wrapper').html());
                applyColumnToggles();
            },
            complete: function() {
                $('#pelaksanaanpelatihan-table').removeClass('d-none');
