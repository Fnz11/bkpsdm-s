<script>
    ...
    function fetchData(page = 1) {
        $('#jenisasn-table').addClass('d-none');
        $('#loading-spinner').removeClass('d-none');
        $('.pagination a').addClass('disabled');

        $.ajax({
            url: "{{ route('dashboard.pelatihan.jenisasn') }}",
            type: "GET",
            data: {
            ...
            success: function(response) {
                $('#jenisasn-table').html($(response).find('#jenisasn-table').html());
                $('#pagination-wrapper').html($(response).find('#pagination-wrapper').html());
                applyColumnToggles();
            },
            complete: function() {
                $('#jenisasn-table').removeClass('d-none');
