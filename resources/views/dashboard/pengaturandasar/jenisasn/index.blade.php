    ...
    <div class="card-header border-0 py-3 bg-transparent">
                ...
                <div class="d-flex flex-column flex-md-row align-items-md-center gap-2">
                    <button class="btn btn-primary px-3 d-inline-flex align-items-center gap-2"
                        onclick="openDynamicModal({
                            title: 'Tambah Jenis ASN',
                            method: 'POST',
                            icon: 'bi-plus-circle',
                            action: '{{ route('dashboard.pelatihan.jenisasn.store') }}',
                            fields: [
                                {
                                    name: 'kode_jenisasn',
                        ...
                        <i class="bi bi-plus-circle me-1"></i> Tambah Jenis ASN
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                @include('dashboard.pengaturandasar.jenisasn._table', [
                    'jenisasns' => $jenisasns,
                ])
            </div>
        </div>
        ...
        <div id="pagination-wrapper" class="card-footer px-5 py-3">
            {{ $jenisasns->links('pagination::bootstrap-5') }}
        </div>
    </div>
@endsection
...
@section('scripts-pengaturandasar')
    ...
    @if (session('open_modal') && session('modal_fields'))
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                openDynamicModal({
                    title: "{{ session('modal_action') === 'edit' ? 'Edit Jenis ASN' : 'Tambah Jenis ASN' }}",
                    method: "{{ session('modal_action') === 'edit' ? 'PUT' : 'POST' }}",
                    action: "{{ session('modal_action') === 'edit' ? route('dashboard.pelatihan.jenisasn.update', old('id', session('data_edit.id') ?? '')) : route('dashboard.pelatihan.jenisasn.store') }}",
                    data: @json(old()),
    ...
    @endif
    @include('dashboard.pengaturandasar.jenisasn.script')
@endsection
