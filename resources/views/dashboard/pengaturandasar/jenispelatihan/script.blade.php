<script>
    ...
    function fetchData(page = 1) {
        $('#jenispelatihan-table').addClass('d-none');
        $('#loading-spinner').removeClass('d-none');
        $('.pagination a').addClass('disabled');

        $.ajax({
            url: "{{ route('dashboard.pelatihan.jenispelatihan') }}",
            type: "GET",
            data: {
            ...
            success: function(response) {
                $('#jenispelatihan-table').html($(response).find('#jenispelatihan-table').html());
                $('#pagination-wrapper').html($(response).find('#pagination-wrapper').html());
                applyColumnToggles();
            },
            complete: function() {
                $('#jenispelatihan-table').removeClass('d-none');
