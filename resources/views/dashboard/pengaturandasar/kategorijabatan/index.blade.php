...
@section('title', 'Kategori Jabatan')
    ...
    <div class="card border-0 shadow-sm p-3">
                ...
                <div class="d-flex flex-column flex-md-row align-items-md-center gap-2">
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="columnDropdown">
                            @foreach (['no', 'kode', 'kategori', 'aksi'] as $col)
                                <li>
                ...
                <div class="d-flex flex-column flex-md-row align-items-md-center gap-2">
                    <button class="btn btn-primary px-3 d-inline-flex align-items-center gap-2"
                        onclick="openDynamicModal({
                            ...
                            method: 'POST',
                            icon: 'bi-plus-circle',
                            action: '{{ route('dashboard.pelatihan.kategorijabatan.store') }}',
                            fields: [
                                {
                        ...
                        <i class="bi bi-plus-circle me-1"></i> Tambah Kategori Jabatan
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                @include('dashboard.pengaturandasar.kategorijabatan._table', [
                    'kategorijabatans' => $kategorijabatans,
                ])
            </div>
        </div>
        
        <div id="pagination-wrapper" class="card-footer px-5 py-3">
            {{ $kategorijabatans->links('pagination::bootstrap-5') }}
        </div>
    </div>
@endsection
...
@section('scripts-pengaturandasar')
    ...
    @if (session('open_modal') && session('modal_fields'))
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                openDynamicModal({
                    title: "{{ session('modal_action') === 'edit' ? 'Edit Kategori Jataban' : 'Tambah Kategori Jataban' }}",
                    method: "{{ session('modal_action') === 'edit' ? 'PUT' : 'POST' }}",
                    action: "{{ session('modal_action') === 'edit' ? route('dashboard.pelatihan.kategorijabatan.update', old('id', session('data_edit.id') ?? '')) : route('dashboard.pelatihan.kategorijabatan.store') }}",
                    data: @json(old()),
    ...
    @endif
    @include('dashboard.pengaturandasar.kategorijabatan.script')
@endsection
