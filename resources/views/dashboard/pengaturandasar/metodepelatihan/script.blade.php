<script>
    ...
    function fetchData(page = 1) {
        $('#metodepelatihan-table').addClass('d-none');
        $('#loading-spinner').removeClass('d-none');
        $('.pagination a').addClass('disabled');

        $.ajax({
            url: "{{ route('dashboard.pelatihan.metodepelatihan') }}",
            type: "GET",
            data: {
            ...
            success: function(response) {
                $('#metodepelatihan-table').html($(response).find('#metodepelatihan-table').html());
                $('#pagination-wrapper').html($(response).find('#pagination-wrapper').html());
                applyColumnToggles();
            },
            complete: function() {
                $('#metodepelatihan-table').removeClass('d-none');
