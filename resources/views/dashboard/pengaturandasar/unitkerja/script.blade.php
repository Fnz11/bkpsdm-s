<script>
    ...
    function fetchData(page = 1) {
        $('#unitkerja-table').addClass('d-none');
        $('#loading-spinner').removeClass('d-none');
        $('.pagination a').addClass('disabled');

        $.ajax({
            url: "{{ route('dashboard.pelatihan.unitkerja') }}",
            type: "GET",
            data: {
            ...
            success: function(response) {
                $('#unitkerja-table').html($(response).find('#unitkerja-table').html());
                $('#pagination-wrapper').html($(response).find('#pagination-wrapper').html());
                applyColumnToggles();
            },
            complete: function() {
                $('#unitkerja-table').removeClass('d-none');
